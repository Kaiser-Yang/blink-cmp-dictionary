...
--- @param items table
local function process_completion_item(leading, match, context, items)
	items[match] = {
		label = match.label,
		insertText = leading .. match.insert_text,
		filterText = leading .. match.insert_text,
		kind = require("blink.cmp.types").CompletionItemKind[match.kind_name] or 0,
		data = match.data,
	}
	if utils.get_option(dictionary_source_config.capitalize_first, context, match) then
		items[match].label = utils.capitalize(match.label, false)
		items[match].insertText = utils.capitalize(match.insert_text, false)
	end
	if utils.get_option(dictionary_source_config.capitalize_whole_word, context, match) then
		items[match].label = utils.capitalize(match.label, true)
		items[match].insertText = utils.capitalize(match.insert_text, true)
	end
	if utils.get_option(dictionary_source_config.decapitalize_first, context, match) then
		items[match].label = utils.decapitalize(match.label, false)
		items[match].insertText = utils.decapitalize(match.insert_text, false)
	end
	if utils.get_option(dictionary_source_config.decapitalize_whole_word, context, match) then
...
function DictionarySource:get_completions(context, callback)
	...
	local leading = utils.get_option(dictionary_source_config.get_leading, context, prefix)
		...
		fallback.load_dictionaries(
			...
			function(return_code, standard_error)
				...
				if return_code ~= 0 then
					if
						dictionary_source_config.on_error
						and dictionary_source_config.on_error(return_code, standard_error)
					then
	...
	if utils.truthy(files) then
		utils.read_dictionary_files_async(files, function(return_code, standard_error, content)
			...
			if return_code ~= 0 then
				if
					dictionary_source_config.on_error and dictionary_source_config.on_error(return_code, standard_error)
				then
					-- on_error returned true, stop processing
