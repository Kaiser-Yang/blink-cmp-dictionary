...
do
	...
	word_pattern = vim.lpeg.Ct((non_word_character ^ 0 * vim.lpeg.C(word_character ^ 1) * non_word_character ^ 0) ^ 0)
end
...
local function default_on_error(return_value, standard_error)
	if utils.truthy(standard_error) then
		vim.schedule(function()
			log.warn(
				"Dictionary file operation failed",
				"\n",
				"with error code:",
				return_value,
				"\n",
...
return {
	...
	on_error = default_on_error,
	get_leading = default_get_leading,
}
